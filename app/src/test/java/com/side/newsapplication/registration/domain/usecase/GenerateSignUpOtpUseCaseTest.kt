...
class GenerateSignUpOtpUseCaseTest {
    ...
    private lateinit var generateOtpUseCase: GenerateSignUpOtpUseCase
    ...
    fun setUp() {
        ...
        generateOtpUseCase = GenerateSignUpOtpUseCase(mockedContext, apiServices)
    }
    @AfterEach
    fun tearDown() {
    ...
    fun `validate network error`() = runTest {
        ...
        Truth.assertThat(
            (response is GenerateSignUpOtpUseCase.GenerateSignUpOtpUseCaseResult.StateSError)
    ...
    fun `validate error response`() = runTest {
        ...
        val response = generateOtpUseCase()
        println("network error test response = $response")
        Truth.assertThat(response is GenerateSignUpOtpUseCase.GenerateSignUpOtpUseCaseResult.StateSError).isTrue()
    }
    @Test
    fun `validate session timeout error response`() = runTest {
        ...
        val response = generateOtpUseCase()
        println("network error test response = $response")
        Truth.assertThat(response is GenerateSignUpOtpUseCase.GenerateSignUpOtpUseCaseResult.StateSessionTimeout).isTrue()
    }
    @Test
    ...
    fun `validate success response`() = runTest {
        ...
        val response = generateOtpUseCase()
        println("network error test response = $response")
        Truth.assertThat(response is GenerateSignUpOtpUseCase.GenerateSignUpOtpUseCaseResult.StateSuccess).isTrue()
    }
}