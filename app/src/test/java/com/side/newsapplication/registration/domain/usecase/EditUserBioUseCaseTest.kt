...
class EditUserBioUseCaseTest {
    ...
    private lateinit var bioSetupUseCase: EditUserBioUseCase
    ...
    fun setUp() {
        ...
        bioSetupUseCase = EditUserBioUseCase(apiServices)
    }
    ...
    fun `given bioInfo is null when validationBioSetup then return BioFailedEmpty`() {
        ...
        Truth.assertThat(
            result is EditUserBioUseCase.BioFailedValidation.BioFailedEmpty
        ).isTrue()
    }
    ...
    fun `given bioInfo has less than 3 chars when validationBioSetup then return MinBioFailed`() {
        ...
        Truth.assertThat(
            result is EditUserBioUseCase.BioFailedValidation.MinBioFailed
        ).isTrue()
    }
    ...
    fun `given bioInfo has more than 3 chars when validationBioSetup then return Success`() {
        ...
        Truth.assertThat(
            result is EditUserBioUseCase.BioFailedValidation.Success
        ).isTrue()
    }
    ...
    fun `validate session timeout error response`() = runTest {
        ...
        val response = bioSetupUseCase("fake bio")
        println("network error test response = $response")
        Truth.assertThat(response is EditUserBioUseCase.BioSetupResult.StateSessionTimeout).isTrue()
    }
    ...
    fun `validate success response`() = runTest {
        ...
        val response = bioSetupUseCase("fake api")
        println("network error test response = $response")
        Truth.assertThat(response is EditUserBioUseCase.BioSetupResult.Success).isTrue()
    }
