...
class GenerateForgotPasswordOtpUseCaseTest {
    ...
    private lateinit var generateOtpUseCase: GenerateForgotPasswordOtpUseCase
    ...
    fun setUp() {
        ...
        generateOtpUseCase = GenerateForgotPasswordOtpUseCase(mockedContext, apiServices)
    }
    ...
    fun `validate network error`() = runTest {
        ...
        apiServices =
            Retrofit.Builder()
                ...
                .build().create(ApiServices::class.java)
        generateOtpUseCase = GenerateForgotPasswordOtpUseCase(mockedContext, apiServices)
        val response = generateOtpUseCase("", "")
        println("network error test response = $response")
        Truth.assertThat(
            (response is GenerateForgotPasswordOtpUseCase.GenerateForgotPasswordOtpUseCaseResult.StateSError)
    ...
    fun `validate error response`() = runTest {
        ...
        val response = generateOtpUseCase("", "")
        println("network error test response = $response")
        Truth.assertThat(response is GenerateForgotPasswordOtpUseCase.GenerateForgotPasswordOtpUseCaseResult.StateSError)
            .isTrue()
    }
    ...
    fun `validate session timeout error response`() = runTest {
        ...
        val response = generateOtpUseCase("", "")
        println("network error test response = $response")
        Truth.assertThat(response is GenerateForgotPasswordOtpUseCase.GenerateForgotPasswordOtpUseCaseResult.StateSessionTimeout)
            .isTrue()
    }
    ...
    fun `validate error response with google mail sign in Id`() = runTest {
        ...
        mockResponse.setBody(
            gson.toJson(
                BaseResponseModel(
                    status = ApiConstants.RESPONSE_FAIL,
                    data = GenerateForgotPasswordResponseModel(
                        authCode = null,
                        email = null
                    ),
                    statusCode = 404,
    ...
    fun `validate success response`() = runTest {
        ...
        val response = generateOtpUseCase("", "")
        println("network error test response = $response")
        Truth.assertThat(response is GenerateForgotPasswordOtpUseCase.GenerateForgotPasswordOtpUseCaseResult.StateSuccess)
            .isTrue()
    }
}