...
class ValidateSignUpOtpUseCaseTest {
    ...
    private lateinit var validateOtpUseCase: ValidateSignupOtpUseCase
    ...
    fun setUp() {
        ...
        validateOtpUseCase = ValidateSignupOtpUseCase(mockedContext, apiServices)
    }
    @AfterEach
    fun tearDown() {
    ...
    fun `validate network error`() = runTest {
        ...
        Truth.assertThat(
            (response is ValidateSignupOtpUseCase.ValidateSignUpOtpUseCaseResult.StateSError)
    ...
    fun `validate error response`() = runTest {
        ...
        val response = validateOtpUseCase("")
        println("network error test response = $response")
        Truth.assertThat(response is ValidateSignupOtpUseCase.ValidateSignUpOtpUseCaseResult.StateSError).isTrue()
    }
    @Test
    fun `validate session timeout error response`() = runTest {
        ...
        val response = validateOtpUseCase("")
        println("network error test response = $response")
        Truth.assertThat(response is ValidateSignupOtpUseCase.ValidateSignUpOtpUseCaseResult.StateSessionTimeout).isTrue()
    }
    @Test
    fun `validate error response with google mail sign in Id`() = runTest {
        ...
        mockResponse.setBody(
            gson.toJson(
                BaseResponseModel(
                    status = ApiConstants.RESPONSE_FAIL,
                    data = ValidateSignUpOtpResponseModel(
                        status = null
                    ),
                    statusCode = 404,
                    message = null
    ...
    fun `validate success response`() = runTest {
        ...
        val response = validateOtpUseCase("")
        println("network error test response = $response")
        Truth.assertThat(response is ValidateSignupOtpUseCase.ValidateSignUpOtpUseCaseResult.StateSuccess).isTrue()
    }
}