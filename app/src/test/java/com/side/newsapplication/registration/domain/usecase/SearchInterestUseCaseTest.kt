...
class SearchInterestUseCaseTest {
    @get:Rule
    ...
    private lateinit var searchInterestUseCase: SearchInterestUseCase
    ...
    fun setUp() {
        ...
        searchInterestUseCase = SearchInterestUseCase(mockedContext, apiServices)
    }
    ...
    fun `validate network error`() = runTest {
        ...
        val response = searchInterestUseCase("", "0")
        Truth.assertThat(response is SearchInterestUseCase.SearchInterestUseCaseResult.StateError)
            .isTrue()
    ...
    fun `validate error response`() = runTest {
        coEvery { apiServices.searchInterests("", "") } returns ErrorResponseModel(
            data = null
        )
        val testResult = searchInterestUseCase("", "")
        Truth.assertThat(testResult is SearchInterestUseCase.SearchInterestUseCaseResult.StateError)
            .isTrue()
    }
    ...
    fun `validate session timeout error response`() = runTest {
        ...
        val testResult = searchInterestUseCase("", "")
        Truth.assertThat(testResult is SearchInterestUseCase.SearchInterestUseCaseResult.StateSessionTimeout)
            .isFalse()
    }

    @Test
    fun `validate success`() = runTest {
        coEvery { apiServices.searchInterests(any(), any()) } returns SuccessResponseModel(
            data = arrayListOf()
        )
        val testResult = searchInterestUseCase("", "0")
        println("test result = $testResult")
        Truth.assertThat(testResult is SearchInterestUseCase.SearchInterestUseCaseResult.StateSuccess)
            .isTrue()
    }
