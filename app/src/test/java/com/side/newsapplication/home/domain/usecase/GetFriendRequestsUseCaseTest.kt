...
class GetFriendRequestsUseCaseTest {
    ...
    private lateinit var useCase: GetFriendRequestsUseCase
    ...
    fun setUp() {
        ...
        useCase = GetFriendRequestsUseCase(apiServices)
    }

    @Test
    fun `validate network error`() = runTest {
        coEvery { apiServices.getFriendingRequest(any()) } throws NoNetworkException()
        val testResult: GetFriendRequestsUseCase.GetFriendRequestsUseCaseResult = useCase.invoke("")
    ...
    fun `validate error response`() = runTest {
        coEvery { apiServices.getFriendingRequest(any()) } returns BaseResponseModel(
        ...
        Truth.assertThat(
            (testResult is GetFriendRequestsUseCase.GetFriendRequestsUseCaseResult.StateError && testResult.response?.status == ApiConstants.RESPONSE_FAIL)
        ).isTrue()
    }
    ...
    fun `validate session timeout error response`() = runTest {
        ...
        val testResult = useCase.invoke("")
        Truth.assertThat(testResult is GetFriendRequestsUseCase.GetFriendRequestsUseCaseResult.StateSessionTimeout && testResult.response?.statusCode == ResponseInterceptor.UNAUTHENTICATED_STATUS_CODE)
            .isTrue()
    }

    @Test
    fun `validate success response`() = runTest {
        coEvery { apiServices.getFriendingRequest(any()) } returns SuccessResponseModel()
        val testResult = useCase.invoke("")
        Truth.assertThat(testResult is GetFriendRequestsUseCase.GetFriendRequestsUseCaseResult.StateSuccess && testResult.response.status == ApiConstants.RESPONSE_SUCCESS)
            .isTrue()
    }
